...
2. Supports purging Label Noise candidates from CSV generated by analyze_misclassifications.py.
3. MOVES all discarded images to a dedicated folder `discarded_images/` preserving class and split structure for subsequent inspection.
4. Reports precise counts of cleaned and preserved images per split and digit.
"""
...
def purge_label_noise_candidates(
    csv_path: Path = config.MODELS_DIR / "label_noise_candidates.csv",
    dataset_dir: Path = config.DATASET_DIR,
    risk_categories: list = ["LABEL_NOISE_CANDIDATE"]
):
    """
    Reads candidates generated by analyze_misclassifications.py and moves high-risk
    noise candidates out of the active dataset into discarded_images/v1.2_label_noise/
    """
    csv_path = Path(csv_path)
    if not csv_path.exists():
        print(f"Error: Candidate CSV {csv_path} not found. Run analyze_misclassifications.py first.")
        return

    df = pd.read_csv(csv_path)
    targets = df[df["Risk_Category"].isin(risk_categories)]
    
    print("=" * 65)
    print(f"  Purging High-Risk Label Noise & Defective Candidates")
    print(f"  Target Categories: {risk_categories}")
    print(f"  Found {len(targets)} candidate samples to isolate.")
    print("=" * 65)

    discard_dest = Path(dataset_dir) / "discarded_images" / "v1.2_label_noise"
    discard_dest.mkdir(parents=True, exist_ok=True)
    
    moved_count = 0
    # Search in dataset splits
    for _, row in targets.iterrows():
        true_digit = int(row["True_Label"])
        sample_idx = int(row["Sample_Idx"])
        
        # Look for images matching the pattern in val or train
        for split in ["val", "train", "test"]:
            digit_dir = Path(dataset_dir) / split / str(true_digit)
            if not digit_dir.exists():
                continue
            files = sorted(list(digit_dir.glob("*")))
            if sample_idx < len(files):
                f = files[sample_idx]
                dest_dir = discard_dest / split / str(true_digit)
    ...
    print(f"--> Isolated {moved_count} label noise candidates to: {discard_dest}")
