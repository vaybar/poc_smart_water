...
### 🔹 Fase 5: Escalado de Capacidad Convolucional a $\alpha=0.50$ (`EXP_009` - NUEVO RÉCORD HISTÓRICO 🏆)
- **Hipótesis:** Dado que $\alpha=0.35$ consumía apenas 14.24 KB Flash (5.5% del límite), duplicar la capacidad convolucional a **$\alpha = 0.50$** otorgará la capacidad de abstracción de características para discriminar sutilezas de curvatura entre '7' vs '3' y '9' vs '8' sin superar las restricciones de la ESP32-S3.
- **Resultado:** **95.75% Acc INT8 (+3.30% de mejora absoluta)**.
  - **Pérdida por Cuantización:** Inexistente (**-0.20%**, el modelo INT8 supera al Float32 de 95.55%).
  - **Resolución de Confusión '7' $\rightarrow$ '3':** Reducida de 13 muestras erróneas a solo **2 muestras** (85% de reducción).
  - **Recall del '7':** Saltó de **85.4% a 96.1%** ($F1 = 0.954$).
  - **Equilibrio Global:** Todas las clases superan el **92.7% de F1-Score**.
  - **Consumo Hardware:** **16.85 KB Flash** (solo 6.5% del límite de 256 KB) y **15.05 KB RAM Arena** (37.6% del límite de 40 KB).
  - **Latencia:** **0.042 ms / imagen** (más de 24,000 FPS en simulación Python).
...
| `EXP_009` | 2026-09-11 20:06 | 0.50 | `64x32x1` | 30 | 32 | 95.55% | **95.75%** | -0.20% | 0.957 | 0.957 | 0.042 | **16.85 KB** | **15.05 KB** | [📄 Ver Reporte](experiments/EXP_009/report.md) | Escalado de capacidad a alpha=0.50 (Salto histórico a 95.75% Acc INT8) |
