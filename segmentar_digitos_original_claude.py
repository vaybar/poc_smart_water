...
Dado un recorte rectangular del dial de un medidor, segmenta los 6 dígitos
...
Uso:
    ...
    # Con número esperado distinto a 6
...
def preprocesar(imagen: np.ndarray) -> np.ndarray:
    ...
    Pasos:
      1. Escala de grises
      2. CLAHE — mejora contraste local (útil para imágenes oscuras o reflejadas)
      3. Binarización adaptativa — dígitos en negro, fondo en blanco
      4. Inversión — dígitos en blanco (255), fondo en negro (0)
         para que la proyección sume los píxeles de los dígitos
    ...
    binaria = cv2.adaptiveThreshold(
        ...
        cv2.THRESH_BINARY_INV,   # dígitos=blanco, fondo=negro
        blockSize=15,
        C=8,
    )

    # Pequeño cierre morfológico para unir trazos fragmentados
...
def encontrar_cortes(
    ...
    Estrategia:
      1. Suavizar la proyección para eliminar ruido
      2. Probar múltiples umbrales hasta encontrar exactamente n_digitos segmentos
      3. Si ningún umbral da el resultado exacto → fusionar o dividir
      4. Si todo falla → fallback a división uniforme

    La búsqueda adaptativa de umbral resuelve casos donde el dial tiene
    mucho ruido o reflejos que elevan la línea base de la proyección,
    haciendo que el umbral fijo del 5% clasifique todo como activo.
    ...
    # Búsqueda adaptativa: probar umbrales del 5% al 60% del máximo
    # para encontrar el que produce exactamente n_digitos segmentos
    ...
    for pct in [0.05, 0.10, 0.15, 0.20, 0.30, 0.40, 0.50, 0.60]:
        ...
        if diff == 0:
            break   # encontramos exactamente los n dígitos
...
def segmentar(
    ...
    Returns:
        ...
        cortes    : lista de (x0, x1) usados para cada recorte
